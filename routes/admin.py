...
from core import supabase, RESEND_API_KEY, RESEND_FROM


def _enviar_email_rover(destinatario: str, nome_rover: str, codigo: str) -> bool:
    """Envia em background via Resend API (HTTPS — funciona no Render)."""
...
def admin_add_rover():
    ...
    try:
        ...
        return jsonify({
            ...
            'message':       f'Rover "{nome}" registado. Email {"enviado" if enviado else "não enviado"}.'
        })
    except Exception as e:
        print(f'[ADD ROVER] {e}')
